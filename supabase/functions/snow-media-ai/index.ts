...
PLEX ACCESS (SMC 1.7.1 and later): Plex is included with an active DreamStreams or VibezTV account, and there is NO code to send. In the SMC app: open the Player, sign into Live TV with the streaming username and password, then open Movies & Shows → Plex — it connects by itself. If the Plex screen shows "Sign into Live TV first", they are not signed into Live TV yet. If it says the account "isn't on file with your provider yet", tell them to message Snow Media in the in-app Community/Support with their Live TV username so it can be added; do not send them to plex.tv/link. The "I run my own Plex server" button and plex.tv/link codes are ONLY for people who host their own Plex server. Never mention an access code or a 4-digit code.
