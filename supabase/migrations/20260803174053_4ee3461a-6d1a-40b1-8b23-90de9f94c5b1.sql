...
BEGIN
  ...
  IF v_customer_id IS NULL AND position('@' in v_ps.panel_username) = 0 THEN
    SELECT count(DISTINCT cs.customer_id), min(cs.customer_id)
...
CREATE OR REPLACE FUNCTION public.capture_player_signin(p_host text, p_username text, p_password text, p_expiration_date date, p_status text, p_max_connections integer, p_is_trial boolean, p_device_id text, p_server_label text, p_supabase_user_id uuid, p_matched_customer_id uuid, p_reason text, p_tenant_code text DEFAULT NULL::text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
