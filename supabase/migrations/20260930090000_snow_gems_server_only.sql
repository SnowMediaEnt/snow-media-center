-- Snow Gems can only be added by the server or an admin.
--
-- Two ways let any signed-in account give itself unlimited gems with nothing
-- but the app's public key and its own login (no change to the app needed):
--
-- 1. update_user_credits only refused writing ANOTHER user's balance. For
--    one's own, 'purchase' or 'refund' added any amount, and a negative
--    'deduction' added too (credits - (-x), and credits >= -x always holds).
--    The 'purchase' row it records also has grant_coins_on_gem_purchase mint
--    Game Lounge coins when a (caller-chosen) paypal id is given.
-- 2. The owner UPDATE policy on profiles has no column limit and nothing
--    guarded the balance columns, so PATCH /profiles {credits: 9999999} worked.
--
-- Legitimate callers, all unchanged:
--   * The app's own charges: 'deduction' of a positive amount from the
--     caller's own balance (useUserProfile.deductCredits, VoiceCommandHost).
--   * Admins (AdminUserManager.adjustCredits): any type, any user.
--   * Edge functions via the service role (ai-tiers premium charge/refund,
--     elevenlabs-tts/-stt, giveaway-bridge gems-paid): auth.uid() is null.
--   * SECURITY DEFINER code (update_user_credits itself, the signup trigger)
--     runs as its owner, not as 'authenticated'.
--   * A user editing their own name, username, phone or email.
