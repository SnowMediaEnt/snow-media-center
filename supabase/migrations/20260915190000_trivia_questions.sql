-- Curated question bank for the Snow Media TV Trivia challenge.
--
-- This table is intentionally separate from knowledge_documents and the
-- private knowledge-base storage bucket. TV clients never receive raw source
-- documents: an admin or service-role job publishes reviewed, four-choice
-- questions here, and the trivia-questions Edge Function returns only the
-- narrow fields required to play.
...
-- A small reviewed launch set makes the endpoint useful immediately. The app
-- also bundles these facts, so a television remains playable while offline.
