 ...
 *  "open", "show me", "find", "pull up", "put on", "start (watching)" …
 *  Group 1 is what follows the verb, group 2 the title alone. Plex on its
 *  own ("open Plex") names no title and is left to the screen words;
 *  "search for … in Plex" is Search. */
const PLEX_TITLE_ASK = /^(?:open(?:\s+up)?|launch|start(?:\s+(?:watching|playing))?|run|go\s+to|take\s+me\s+to|bring\s+up|show(?:\s+me)?|pull\s+up|put\s+on|turn\s+on|switch\s+to|find(?:\s+me)?|get\s+me|queue(?:\s+up)?)\s+((.+?)\s+(?:on|in|from)\s+(?:the\s+)?plex(?:\s+(?:app|server|library))?)$/;
