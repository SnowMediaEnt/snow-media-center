// The html inside Snow Mail: a strict allow-list, no scripts, no styles, no
// links out.
//
// The reader puts it through dangerouslySetInnerHTML in the app's WebView,
// which carries the native bridge (billing, app installs), so nothing from
// the campaign is passed through as markup. The output is rebuilt: every tag
// in it is one written here from the allow-list, with no attributes except an
// https image src, and every other `<` and `>` becomes text. An unfinished
// tag, a comment, an event handler or a javascript: URL cannot survive,
// however the input is written. One pass, linear in the input.
//
// The mail-publish edge function runs the same code before a campaign is
// stored, in supabase/functions/_shared/mailHtml.ts; keep the two identical.
