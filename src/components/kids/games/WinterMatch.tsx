...
type Card = { pair: number; label: string; kind: 'shape' | 'text' };
const SHAPES = ['●', '▲', '■', '♥', '★', '◆'];
export function createWinterCards(tier: KidsGameProps['tier'], level: number): Card[] {
  const pairs = tier === 'little' ? 3 : tier === 'kids' ? 4 : 6;
  const cards: Card[] = [];
  for (let pair = 0; pair < pairs; pair++) {
    const answer = pair + 3 + (level - 1) % 8;
    cards.push({ pair, label: tier === 'little' ? SHAPES[pair] : tier === 'kids' ? `${answer - 1} + 1` : `${answer} × 3`, kind: tier === 'little' ? 'shape' : 'text' });
    cards.push({ pair, label: tier === 'little' ? SHAPES[pair] : String(tier === 'kids' ? answer : answer * 3), kind: tier === 'little' ? 'shape' : 'text' });
  }
...
export default function WinterMatch({ tier, progress, onComplete, onBack, soundOn }: KidsGameProps) {
  ...
  const reveal = (index: number) => {
    ...
    if (isMatch) {
      ...
      setMessage(tier === 'little' ? 'A matching pair! Well spotted.' : `${cards[next[0]].label} matches ${cards[next[1]].label}. Great thinking!`);
  ...
  return <section className="kp-game kp-match" aria-label="Winter Match"><header className="kp-heading"><div><span className="kp-eyebrow">LOOK • REMEMBER • MATCH</span><h2>Winter Match</h2></div><span className="kp-level">Round {level}</span></header>
    <div className="kp-match-info"><p>{tier === 'little' ? 'Find the same shapes.' : 'Match each sum with its answer.'}</p><span>{matched.length} / {cards.length / 2} pairs • {turns} turns</span></div>
    <div className="kp-cards" style={{ gridTemplateColumns: `repeat(${columns}, 1fr)` }}>{cards.map((card, index) => { const revealed = open.includes(index) || matched.includes(card.pair); const found = matched.includes(card.pair); return <button key={index} ref={element => { buttons.current[index] = element; }} tabIndex={focus === index ? 0 : -1} onFocus={() => setFocus(index)} onClick={() => reveal(index)} className={`kp-card ${revealed ? 'is-open' : ''} ${found ? 'is-matched' : ''} ${focus === index ? 'is-focused' : ''}`} aria-label={revealed ? `${card.label}${found ? ', matched' : ''}` : `Hidden card ${index + 1}`} aria-disabled={found}><span className={card.kind === 'shape' && revealed ? `kp-shape kp-shape-${card.pair}` : ''}>{revealed ? card.label : <Snowflake />}</span>{found && <span className="kp-card-check">✓</span>}</button>; })}</div>
